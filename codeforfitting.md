...
#------------------------------------------------------------------------------------------------------------------------------------
#New fitting function for delta epsilon
import numpy as np
from matplotlib.pyplot import cm
from matplotlib import pyplot as plt
import matplotlib
...
def to_complex(real,imag):
    ...
    energy_axis, *data_de1 = Read("de1_values.txt", 9, False)
_, *data_de2 = Read("de2_values.txt", 9, False)
data_de2 = np.array(data_de2)
data_de1 = np.array(data_de1)
data_labels = [1,5,10,20,40,60,80,100]
print(data_de1.shape, data_de2.shape)
...
# for i,c in zip (range(data_de1.shape[0], color)):
    ax[0].plot(energy_axis,data_de1[i,:], 'bo-', c=c, label=data_labels[i])
    ax[1].plot(energy_axis,data_de2[i,:],'bo-',c=c, label=data_labels[i])
    ax[0].legend()
    ax[1].legend()
...
def de_lorentz_w0_real(x,s,w0,t):
    ...
    den = t**2*((w0**2-x**2)**2 + x**2)
    return num/den
# def de_lorentz_Sct_real(x,s,w0,t):
#     num = 2*s*t**2*(w0**2-x**2)
#     den = t**2*(w0**2-x**2)**2+x**2
#     return num/den

# def de_lorentz_Sct_imag(x,s,w0,t):
#     num = 2*s*x*t
#     den = t**2*(w0**2-x**2)+x**2
#     return num/den
...
def drude_imag(x, s, t):
 ...
 def delta_e1(x,Sd,Smir,Wct,Tct):
    ...
    eq += drude_real(x,Sd,Td)
    eq += lorentz_real(x,Smir,Wmir,Tmir)
    eq += de_lorentz_w0_real(x,Sct,Wct,Tct)*(Wct-Wct0)
#     eq += de_lorentz_Sct_real(x,Sct,Wct,Tct)*(Sct-Sct0)
    eq += de_lorentz_t_real(x,Sct,Wct,Tct)*(Tct-Tct0)
    return eq
def delta_e2(x,Sd,Smir,Wct,Tct):
    Td = 7
    ...
    eq += lorentz_imag(x,Smir,Wmir,Tmir)
    eq += de_lorentz_w0_imag(x,Sct,Wct,Tct)*(Wct-Wct0)
#     eq += de_lorentz_Sct_imag(x,Sct,Wct,Tct)*(Sct-Sct0)
    eq += de_lorentz_t_imag(x,Sct,Wct,Tct)*(Tct-Tct0)
    return eq
...
guess = [0.03525463, 0.24854598, 2.20985082, 1.88342405]
multiguess = [[0.3965329,  0.20315181, 2.17820595, 1.44120818],
             [0.3965329,  0.20315181, 2.17820595, 1.44120818],
             [0.3965329,  0.20315181, 2.17820595, 1.44120818],
             [0.3965329,  0.20315181, 2.17820595, 1.44120818],
             [0.3965329,  0.20315181, 2.17820595, 1.44120818]]
#              [.88552803e-01, 0.214, 2.18089719, 1.84826139],
#              [.88552803e-01, 0.214, 2.18089719, 1.84826139]]
#              [.88552803e-03, 6.94166122e-01, 2.17888565, 1.83401925]]
...
for i,c in zip(range(n),color):
    ...
    ax[0].plot(x_data,y_real, 'o',c=c)
    ax[1].plot(x_data,y_imag, 'o',c=c)
    ax[0].set_ylabel('d_Epsilon_1')
    ax[0].set_xlabel('Energy [eV]')
    ax[1].set_ylabel('d_Epsilon_2')
    ax[1].set_xlabel('Energy [eV]')
...
#     popt, cov_x, infodict, mesg, ier = leastsq(residuals, p0, args=(z,x_data), full_output=1)
    ...
    ax[0].plot(x_fit,fit_curve_de1,'-', c=c, label='{} mW'.format(data_labels[i]))
#     ax[0].plot(x_fit, lorentz_real(x_fit,.15,0.5,0.9))
#     ax[0].plot(x_fit, drude_real(x_fit,.19,8))
#     ax[0].plot(x_fit, de_lorentz_w0_real(x_fit,0.8,2.1,1))
    ax[1].plot(x_fit,fit_curve_de2,'-', c=c, label='{} mW'.format(data_labels[i]))
#     ax[1].plot(x_fit, lorentz_imag(x_fit,.15,0.5,0.9))
#     ax[1].plot(x_fit, drude_imag(x_fit,.19,8))
#     ax[1].plot(x_fit, de_lorentz_w0_imag(x_fit,.8,2.1,1))
    ax[0].legend()
    ax[1].legend()
#     res_percent_real = 100*(y_real-(Drude1Lorentz2_real(x_data,*popt)))/y_real
#     res_percent_imag = 100*(y_imag-(Drude1Lorentz2_imag(x_data,*popt)))/y_imag
...
# popt, cov_x, infodict, mesg, ier  = res
print(popt)
f.savefig('de1_de2_values_best fit5.png')
...
for key, val in fit_pars.items():
    ...
    ax[a,b].set_xlabel('Power [mW]')
    i+=1
f.savefig('de1_de2_fit_parameters5.png')
